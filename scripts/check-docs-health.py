#!/usr/bin/env python3
"""Read-only documentation health checks for CrashCenter."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="CrashCenter docs health check (read-only).")
...
def crlf_findings(files: list[Path]) -> list[Finding]:
    out: list[Finding] = []
    for path in files:
        if b"\r\n" in path.read_bytes():
            out.append(Finding(repo_rel(path), "CRLF line endings (prefer LF)"))
    return out


def status_md_findings() -> list[Finding]:
    path = REPO_ROOT / "dev/progress/status.md"
    if not path.is_file():
        return []
    ...
    if line_count > 200:
        return [
            Finding(
                repo_rel(path),
                f"line count {line_count} exceeds 200 (archive old Recent Sessions)",
            )
        ]
    return []
...
def main() -> int:
    ...
    crlf = crlf_findings(files)
    status_lines = status_md_findings()
    if crlf:
        warnings["crlf"] = crlf
    if status_lines:
        warnings["status_md"] = status_lines
    ...
    if args.format == "json":
        ...
        print("Docs health check (CrashCenter)")

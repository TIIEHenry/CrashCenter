...
phase: N/A
updated: 2026-06-20
summary: "待决 ADR、实现与架构文档漂移、需新建/修订的架构文档清单与推荐顺序"
---
...
| **架构文档 proposed/draft** | 12+ | 4D+ 规格已有但未 accepted；design/ 组件库仍为 draft |
...
#### D-04 · CrashLogRepository 读口契约（实现已简化）

| 项 | 内容 |
|----|------|
| **背景** | 文档定义 `observeChanges()`、`clear()`、`deleteById()`、`applyRetention()`；[FileCrashLogRepository.kt](../../app/src/main/java/nota/android/crash/xp/app/data/CrashLogRepository.kt) **仅** `getAll/getById/getCount`，LRU 200 + mtime 失效，**无 Flow** |
| **分叉** | A) 修订 crash-data-layer 对齐 as-built，4D 再扩展接口；B) 补全 Repository 接口再写 4D UI；C) 引入 Paging3 为唯一列表读口，废弃 offset 扫描 |
| **影响** | CrashHistoryPagingSource、StatsAggregator、清空历史、导出 |
| **建议** | **先修订 crash-data-layer（as-built 节）**，4D 启动前 **ADR-019** 定 *清空/retention 是否经 Repository* |
...
| **建议** | 若 4G-MVP 需要 `threadName`/`causeClasses` → 在 crash-logging 增 **字段版本** `schemaVersion` 即可，不必单独 ADR；若改 JSONL 破坏性 → ADR-021 |

#### D-08 · ADR-013 crash_id 通知：实施窗口

| 项 | 内容 |
|----|------|
| **背景** | ADR-013 **accepted**，标注 Phase 4E；当前通知仍传 `Exception` extra |
| **分叉** | 4E 统一改 vs 4C-β 详情就绪后即改（Repository.getById 已可用） |
| **建议** | 早改可减 Binder 风险；须同步 [crash-notification.md](../../docs/architecture/crash-notification.md) 与 ActivityCrashInfo 回退逻辑 |
...
| 文档 | 问题 | 状态 |
|------|------|------|
| [ui-routing.md](../../docs/architecture/ui-routing.md) | §As-built 2026-06 MainShell | ✅ 2026-06-20 |
| [crash-data-layer.md](../../docs/architecture/crash-data-layer.md) | as-built 三方法 + Paging | ✅ 2026-06-20 |
| [overview.md](../../docs/architecture/overview.md) | SDK 37、ServiceLocator、Paging | ✅ 2026-06-20 |
| [AGENTS.md](../../AGENTS.md) | compileSdk/targetSdk 37 | ✅ 2026-06-20 |
| [architecture-optimization.md](../../docs/architecture/architecture-optimization.md) | §9.3 ADR 编号冲突 | ✅ 2026-06-20 |
| [crash-history-ui.md](../../docs/architecture/crash-history-ui.md) | Paging、CrashEventBinder | ✅ 2026-06-20 |
| [status.md](../progress/status.md) | Snapshot 滞后 | ✅ 2026-06-20 |
...
| [adb-logcat-analysis.md](../../docs/architecture/adb-logcat-analysis.md) | 查状态 | 4F 脚本立项 |
| `docs/design/**` | draft | 与 design-system.md 合并推广或标记 *reference only* |
...
| ADR-013 crash_id | 通知代码路径未改 — 标 implementation gap |
...
- [decisions/INDEX via docs/INDEX.md](../../docs/INDEX.md) — ADR 索引

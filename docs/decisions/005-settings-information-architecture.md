...
phase: 3
updated: 2026-06-19
summary: "配置 tab 单屏高密度：全局 Chip 与列表同屏；Phase 3G 受管列表（ADR-015）；Phase 4C+ 观测域 2-tab"
---
...
## 修订（2026-06-19 · ADR-015）

[ADR-015](../decisions/015-managed-apps-intervention-rules.md) 修订配置 tab **列表范围**，**不**改变「单屏高密度」原则：

| 项 | ADR-005 原假设 | ADR-015 修订 |
|----|----------------|--------------|
| 列表内容 | 全量已安装 app | **仅受管应用**（`managed_packages`） |
| 添加入口 | 无（被动枚举） | Toolbar + 空态 → **Half Sheet Picker** |
| 行内 Switch | 写 `package_list` 黑名单 | 写 **干预规则**；OFF→ON 可隐式 `CATCH_ALL` |
| 过滤 Chip | 全部 / 已应用 / 未应用 | 全部 / **已启用** / **待配置** |
| Toolbar 批量 | 全选 / 全不选 | **移除**（改为 Picker 多选添加） |
| 细配 | 无 | L3 **应用干预编辑页** |

**不变**：全局 FilterChip 三件套、搜索、状态条、包可见性 banner 仍与列表 **同屏**；配置 tab 不增设 bottom tab。

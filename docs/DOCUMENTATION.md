...
│   ├── active/               # 进行中 phase
│   └── archive/              # 已完成 phase
├── iterations/<subsystem>/   # 实现日志（非 ADR）
├── plans/                    # 实施计划
└── archive/                  # 过时文档
```
...
| 历史 | → `dev/progress/archive/status-sessions-*.md` | 不写入 status |
| iter 日志 | → `dev/iterations/` 或 `dev/archive/` | 禁止写入 status |
...
| 写什么 | 放哪里 |
|--------|--------|
| 架构取舍、跨模块契约、推翻 ADR | `docs/decisions/NNN-topic.md` |
| phase 踩坑、验证、实现细节 | `dev/iterations/<subsystem>/` |
...
除非用户明确要求（`docs/README.md`、`dev/README.md` 等导航入口除外）。

### 规则 11：方案与实施 commit 分离
...
| Root 参考（Phase 4） | [reference/root-service-patterns.md](reference/root-service-patterns.md) |
| 设备验收 | `dev/verification/README.md` |
| commit 前对齐什么 | 规则 3a |

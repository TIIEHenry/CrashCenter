...
> 适用模块：`:app`（4B-α：`CrashLogProvider` 已实现；hook 侧 `ProviderBackend`）
...
## As-built：CrashLogProvider（4B-α）

`nota.android.crash.xp.app.CrashLogProvider` 已实现 ADR-007 Fallback B。

### Manifest

```xml
<provider
    android:name=".CrashLogProvider"
    android:authorities="nota.android.crash.xp.app.crashlog"
    android:exported="true" />
```

- **无** `android:permission`（避免 [signature 权限悖论](#signature-权限悖论fallback-b-关键缺陷)）
- 仅实现 `insert`；`query` / `update` / `delete` 返回空

### URI 与 ContentValues

| 项 | 值 |
|----|-----|
| Authority | `nota.android.crash.xp.app.crashlog` |
| Path | `events` |
| URI | `content://nota.android.crash.xp.app.crashlog/events` |
| `COLUMN_PAYLOAD` | 单行 JSON（`CrashEvent.toJsonLine()`） |
| `COLUMN_PACKAGE_NAME` | 声明方包名，须与 payload 内 `packageName` 一致 |

`ProviderBackend` 调用：

```kotlin
contentResolver.insert(CrashLogContract.EVENTS_URI, ContentValues().apply {
    put(COLUMN_PAYLOAD, stamped.toJsonLine())
    put(COLUMN_PACKAGE_NAME, event.packageName)
})
```

### insert 流程

1. `UriMatcher` 匹配 `events` path
2. `validateCaller(claimedPackage)`：`Binder.getCallingUid()` → `getPackagesForUid` 须含 `claimedPackage`（模块同 UID 调用放行）
3. `checkRateLimit()`：每 UID **60s 内最多 30 次** insert
4. `payload.length <= 64 * 1024`（字符）
5. `CrashEvent.fromJson(payload)`；`event.packageName == claimedPackage`
6. `CanonicalJsonlWriter.append(FileCrashLogRepository.eventsFile(ctx), stamped)` — stamp `provider_insert` 到 `backendWritten`
7. 返回 `content://…/events/{event.id}`；异常 → `null`（silent）

### Retention

与 hook 直写共用 `CanonicalJsonlWriter`：**500 条 / 8 MB** 先到者删最旧行（Provider 进程内执行，无单独 pref）。

### 与 4B-α Coordinator 的关系

`CrashLogProvider` 与 `DirectFsBackend` 可能并行写入同一 canonical JSONL；`FileLock` 串行化 append。ingest / dedupe **defer 4B-β**。
...
**编排层（4B-α as-built）**（[ADR-008](../decisions/008-multi-backend-crash-log-storage.md)、[crash-log-backends.md](crash-log-backends.md)）：

```
Phase 1:   RootSu (tier 0) — defer 4B-β
Phase 2:   B Provider ∥ A DirectFs ∥ Relay 并行 — ✅ 4B-α
Module:    CrashLogProvider → CanonicalJsonlWriter — ✅ 4B-α
           RootFs ingest relay → canonical — defer 4B-β
...
sequenceDiagram
    ...
    participant BG as Phase 2 Backends
    participant CP as CrashLogProvider
    participant JSONL as events.jsonl

    Main->>Main: CrashHandler 捕获异常
    Main->>Pipe: onException(...)
    Note over Main: CrashFeedbackFacade（独立 try）
    Pipe->>Coord: logAsync 异步（单线程 executor）
    Coord->>BG: Phase 2 并行 append
    BG->>CP: ProviderBackend → insert
    CP->>JSONL: CanonicalJsonlWriter + FileLock
    BG->>JSONL: DirectFsBackend 直写（若 SELinux 允许）
...
用户从未打开模块 UI 时：`scope_mode` 默认 `false`（hook 全部 app）；`crash_log_enabled` 默认 `true`（`PrefManager.PREF_CRASH_LOG_ENABLED`，4B-α）— hook 仍应尝试写入。
...
当前 Manifest export：`ActivityMain`、`ActivityCrashInfo`、**`CrashLogProvider`**（4B-α）；Provider 须内部 UID 校验与 rate limit（见 [§ As-built](#as-builtcrashlogprovider4b-α)）。
...
**4B-α as-built**：`CrashLogCoordinator` 在单线程 executor 上执行；`CrashFeedbackFacade` 与 Coordinator 各自独立 try/catch，**无** `System.exit`（[ADR-011](../decisions/011-feedback-failure-isolation.md)）。日志失败不影响吞异常。
...
曾参照 [CelestailRuler](../reference/sibling-projects.md#celestailruler) 评估 hook **System Framework**（`system_server` / `android` 包）以改善跨进程写入或集中采集崩溃。**结论：不作为主架构**——干预与观测均须在目标 app 进程；system_server 代写或 AMS crash hook 无法替代 [ADR-007](../decisions/007-crash-log-cross-process-storage.md) 的 Provider fallback，且无法解决已吞 Java 异常的采集。

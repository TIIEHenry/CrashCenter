...
> 主题决策：[ADR-006](../decisions/006-material3-toolchain.md)（M3 defer）

## 概述

CrashCenter Design System 将 **Clarence 生态视觉语言**（Fluent token、信息流/工具密度 UI 模式）与 **Android 实现**（Material Components 2.x + 自定义 token）桥接，并定义可跨域复用的共享组件集合。
...
<style name="AppTheme" parent="Theme.MaterialComponents.Light.NoActionBar">
    ...
    <!-- edge-to-edge + dark icons -->
    <item name="android:statusBarColor">@android:color/transparent</item>
    <item name="android:windowLightStatusBar">true</item>
</style>
```

**不迁移 M3**：当前 `Theme.MaterialComponents.Light.NoActionBar` + Fluent 扩展足够（[ADR-006](../decisions/006-material3-toolchain.md)）。Shell 与 Design System 不依赖 M3 dynamic color 作为前置。
...
| 观测域专有组件不外流 | `CrashStatsCard`（4D）仅统计页 |
...
| Phase 4C-β | `CrashEventRow` 在历史列表复用 |
...
- [crash-history-ui.md](crash-history-ui.md) — 历史列表（`CrashEventRow` 消费方）
...
- [ADR-006](../decisions/006-material3-toolchain.md) — M3 defer

...
CrashCenter 已完成 **干预层**（吞异常）与 **配置 UI**（Phase 3 近完成），**观测层 / 分析层 / 壳层导航** 仍停留在方案文档。当前 `:app` 单模块、扁平包结构在 Phase 4 多后端 IPC、历史 UI、统计与分析并行落地时，易出现 hook 与 UI 耦合、职责边界模糊与测试困难。
...
flowchart TB
    ...
    subgraph target [目标 app 进程 — hook 运行时]
        SH[shouldHandlePackage + XSharedPreferences.reload]
        CH[CrashHandler.insert]
        CB[ExceptionHandler 回调]
        TN[Toast + Notification]
        ACI_INTENT[PendingIntent → ActivityCrashInfo]
        XE --> SH --> CH
        CH --> CB
        CB --> TN
        CB --> ACI_INTENT
    ...
    subgraph module [模块 app 进程 — UI 运行时]
        AM[ActivityMain 单 Activity]
        ACI[ActivityCrashInfo TextView]
    ...
    PREFS -.->|XSharedPreferences 只读| SH
...
| `nota.android.crash.xp.XposedEntry` | hook | 包过滤、hook 安装、通知、self hook |
| `nota.android.crash.xp.PrefManager` | 共享常量 | prefs key（无 Phase 4 观测 key 常量） |
| `nota.android.crash.xp.app.ActivityMain` | 模块 | 配置 UI 全功能单体 |
| `nota.android.crash.ActivityCrashInfo` | 模块 | 通知详情（依赖 `xp.app` 的 SystemBars / ViewBinding） |
...
| CrashEvent JSONL | hook → 模块存储 | ❌ 未实现 |
| 分析结果 | 模块进程 | ❌ 未实现 |
...
| 优先级 | 问题 | 现状证据 | 影响 |
|--------|------|----------|------|
| **P0** | hook 回调与 UI 反馈未分层 | `XposedEntry.hookToGrabCrash` 内联 Toast/Notification；`System.exit(0)` 与日志路径共用 try | Phase 4B 须独立 CrashLog 路径；日志失败不得 exit |
| **P0** | `showNotify` 静态变量 | `XposedEntry` 第 39、61–64 行；多包并发加载互相覆盖 | 通知开关错乱；与 per-package scope 语义冲突 |
| **P1** | 无观测层实现 | 无 `CrashLogCoordinator` / `CrashEvent` | Phase 4 全部 backlog 无代码锚点 |
...
| **P2** | `PrefManager` 缺 Phase 4 键 | 观测 pref 仅在文档 | 实现时易 magic string 漂移 |
...
|--------|---------|------|------|----|----|----|-----|
| 消除 showNotify static + 反馈/日志 try 隔离 | 可选小 PR | **必须** | | | | | |
| PrefKeys 扩展观测键 | | ✅ | | | | | |
| CrashLogBackend + Coordinator | | ✅ | | | | | |
| CrashLogProvider | | ✅ | | | | | |
| RootSu + Ingest + Repository | | | ✅ | | | | |
...
### 8.2 Phase 4B 实施顺序（推荐）

1. `core.CrashEvent` + `PrefKeys`
2. `hook.logging.*` Backend 接口与 Coordinator（先 Phase2 并行，后 root）
3. `CrashCapturePipeline` 接入 `XposedEntry` 回调
4. `CrashLogProvider` + IS-1~IS-6 矩阵
5. `module.ingest.*` + `CrashLogRepository` 读 canonical
6. 文档同步 [crash-logging.md](crash-logging.md) status → accepted（实施 commit 时）
...
| 编号 | 触发条件 |
|------|----------|
| **ADR-009** | 4G-V3 引入端侧 LLM 或云端 API 分析 — 隐私与数据出境边界 |
| （可选）**ADR-010** | 若 Gradle 拆 `:crash-log-root` 独立模块 — libsu 版本与 ProGuard 策略 |

当前优化项 **不强制** 新建 ADR；与 ADR-001/002/007/008 **无冲突**。

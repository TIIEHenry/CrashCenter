...
phase: 4
updated: 2026-06-23
summary: "现状债务清单、目标分层与包结构、Phase 4 落地映射；4D/4E/4F/4G-MVP 部分 as-built"
---
...
CrashCenter 已完成 **干预层**（吞异常）、**配置 UI Shell**（Phase 3/4C-α）、**观测读路径**（4B-α 写入 + 4C-β 历史 + **4D 全局统计 + 4E 导出 + 4F logcat 导入 + 4G-MVP 详情分析** 部分 as-built）。**4B-β ingest 验收、4B-γ CanonicalJsonlStore、单应用观测页、4G 持久化** 仍待落地。当前 `:app` 单模块在 Phase 4 后半段并行实施时，须维持 hook 与 UI 包边界、Repository 读口单一化。
...
| L3 | `PerAppCrashActivity` | 4D+ 单应用观测（**2026-06-23 as-built**） |
...
| RelayMergeBackend id dedupe | | | ✅ | | | | |
| FileCrashLogRepository 读 + 历史 UI | | 读 ✅ | | ✅ | | | |
| MainShell + ConfigFragment | | | | ✅ | | | |
| CrashLogDetail + CodeEditor | | | | ✅ | | | |
| StatsAggregator + 观测 tab 统计 | | | | | ✅ | | |
| PerAppCrashActivity + 统计 TOP 下钻 | | | | | ✅ | | |
| CrashSignature / topClusters / topCategories | | | | | | | ✅ |
| 通知 crash_id | | | | | | ✅ | |
| AnalysisWorker + RuleEngine | | | | | | | ✅（lazy 详情；Worker defer） |

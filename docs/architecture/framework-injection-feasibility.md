...
phase: 4
updated: 2026-06-24
summary: "不采用 System Framework 为主架构；AMS crash/ANR hook 均否决；ANR 走 logcat（ADR-025）；可选 parseQueries 补丁"
---
...
**ANR 观测**：不 hook `AnrHelper` / `ProcessErrorStateRecord`（[ADR-025](../decisions/025-anr-observation-no-framework-hook.md)）；走 [logcat-multi-source.md](logcat-multi-source.md) P2/P3 与可选 `ApplicationExitInfo`。
...
- celestailruler：minSdk 24，compileSdk 36；CrashCenter：minSdk 26，compileSdk 37（见 [AGENTS.md](../../AGENTS.md)）。
...
| # | 用例 | LSPosed / 经典 Xposed | Android 10–16 / minSdk 26 | Scope 要求 | 对 CrashCenter 价值 | 风险 | 维护负担 | 结论 |
|---|------|----------------------|---------------------------|------------|-------------------|------|----------|------|
...
| **6** | **ANR 观测**（hook `AnrHelper` / `ProcessErrorStateRecord`） | 技术上可 hook `system_server` | `AnrHelper`（12+）与 `TimeoutRecord` 每 major 版本漂移 | **必须勾选 System Framework** | **中**（补 Java crash 盲区）但可被 logcat / `ApplicationExitInfo` 覆盖 | system 稳定性；OEM 分叉；抑制 ANR 风险极高 | **高** | ❌ **否决**（[ADR-025](../decisions/025-anr-observation-no-framework-hook.md)） |
...
**用例 6**：ANR 由 `system_server` 超时判定，app 级 `CrashHandler` **无法**感知主线程卡死（无异常）。Framework hook 在观测上有价值，但须 System Framework scope、跟 AOSP 内部 API 绑定，且 **抑制 ANR**（不杀进程/不弹窗）与续命叠加风险过大。采纳 **logcat `system`/`events` buffer**（[logcat-multi-source.md](logcat-multi-source.md)）与可选 **app 内 `ApplicationExitInfo`**，见 [ADR-025](../decisions/025-anr-observation-no-framework-hook.md)。
...
| **永不** | XServiceManager / AMS crash hook / **AMS ANR hook** / framework UEH / Broadcast 写日志 / Framework 抑制 ANR |
...
| **+ AMS ANR hook** | System Framework | 见 [ADR-025](../decisions/025-anr-observation-no-framework-hook.md)；改用 logcat / ApplicationExitInfo |
...
- [logcat-multi-source.md](logcat-multi-source.md) — ANR 线索（system / events buffer）

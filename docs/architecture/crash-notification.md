...
> 适用模块：`:app`
> 源码：`XposedEntry.java`、`CrashHandler.java`、`ActivityCrashInfo.java`
...
通知与拦截是**解耦**的：包可被 hook（续命）但不弹通知；通知失败不应影响吞异常（除 `hookToGrabCrash` 外层 catch 中的 `System.exit` 兜底）。
...
sequenceDiagram
    ...
    participant XE as XposedEntry 回调
    ...
    CH->>XE: handlerException(throwable)
    XE->>XE: Handler(mainLooper).post(...)
    alt showNotify == true
        XE->>Target: Toast
        XE->>NM: notify(随机 id, Notification)
    end
...
目标 app 进程
  Application.onCreate (hook after)
    ...
    → handler 内 Handler(mainLooper).post {
         XposedBridge.log(throwable)
         if (showNotify) {
           Toast（主线程）
           showNotification() → NotificationManager.notify（主线程）
         }
       }
...
`showNotify` 为 `XposedEntry` 内**静态**字段，在 `shouldHandlePackage()` 或 `selfCheck()` 路径上赋值，在异常回调里只读。
...
| 情况 | 行为 |
|------|------|
| `showNotification` 内任意 `Exception` | `XposedBridge.log(e)`，**不**杀进程 |
| `hookToGrabCrash` 回调外层 `catch (Throwable e)` | `System.exit(0)` + log（通知/Toast 逻辑整体失败时的硬兜底） |
| 模块未安装 / Activity 无法启动 | 用户点击通知无效；崩溃 app 仍可能已续命 |

观测层（JSONL）与通知**并行独立**：Phase 4 计划在 `showNotify` 之外始终异步写日志（见 [crash-logging.md](crash-logging.md)）。

---

## 已知实现注意点

1. **`showNotify` 静态字段**：单进程内多 `Application` 极少见；若同一进程多次 `handleLoadPackage` 逻辑重复执行，以最后一次 `shouldHandlePackage` 为准。

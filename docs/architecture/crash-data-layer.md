...
phase: 4
updated: 2026-06-19
summary: "CrashLogRepository 作为 UI 唯一读口；StatsAggregator 聚合统计；retention 轮转与清空策略"
---
...
CrashLogIngestCoordinator
    ├── RootFsBackend (harvest relay)
    └── merge → events.jsonl
                    ↑ append (hook 写入)
                    │
           CrashLogRepository (读)
        ...
        ┌───────────┼───────────┐
        ▼           ▼           ▼
  CrashHistory   StatsAgg   AnalysisWorker
  Fragment       regator    (4G)
```
...
| 策略 | 说明 |
|------|------|
| 全文件扫描 | 顺序读 `events.jsonl`，逐行 JSON parse |
| 内存缓存 | 首次加载后 `List<CrashEvent>` 缓存；文件变更时 invalidate |
| 文件监听 | `FileObserver` 或轮询 `lastModified`；ingest/Provider 写入后通知 `observeChanges` |
| 线程 | `Dispatchers.IO`；UI 层在 ViewModel 内 `viewModelScope` 调用 |
...
| 5000+ | 可选 Room 迁移；Repository 接口不变 |

## StatsAggregator

聚合层，仅依赖 `CrashLogRepository`：

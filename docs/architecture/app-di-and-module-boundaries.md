...
| 依赖 | 实现 | 生命周期 |
|------|------|----------|
| `AppRepositoryInterface` | `AppRepository` | Application 级单例 |
...
| 测试 | 模式 |
|------|------|
| `ConfigViewModelTest` | `FakeAppRepository` implements `AppRepositoryInterface` |
...
- `@Singleton` 绑定 `CrashLogRepository` / `AppRepositoryInterface`

...
phase: 4
updated: 2026-06-23
summary: "CrashEvent JSONL 模型、retention、ingestedFrom；观测/拦截双路径（ADR-023）"
---
...
| **观测层** | 记录每次被捕获的 Java 崩溃（含仅观测与已拦截） | **4B-α 部分 MVP** |
...
Application.onCreate (目标进程)
  ...
  → CrashCapturePipeline.onException(...)
       ├─ shouldIntercept ? logAsync : logSync（观测路径 relay 优先）
...
**结论**：`crash_log_enabled == true` 时每次捕获均尝试持久化（与 Toast / 是否拦截无关）。

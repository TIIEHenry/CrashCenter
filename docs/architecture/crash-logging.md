...
> 适用模块：`:app`（待建 `CrashLogger` 等组件）
> 源码入口（当前）：`XposedEntry.hookToGrabCrash` → `CrashHandler.insert` 回调
...
| **观测层** | 记录每次被拦截的崩溃（结构化字段 + 本地持久化） | **本文档（proposed）** |
...
术语见 [glossary.md](../glossary.md)（**Observation Layer**、**CrashLogger**）。
...
Application.onCreate (目标进程)
  ...
  → XposedEntry 回调（主线程 Handler.post）
       ├─ XposedBridge.log(throwable)          // logcat，非持久化
       └─ 若 showNotify:
            ├─ Toast
            └─ Notification → ActivityCrashInfo（Intent extra 单次 stack）
```

详见 [xposed-entry.md](xposed-entry.md)、[crash-handler.md](crash-handler.md)。

### 当前存储

| 存储 | 用途 | 崩溃历史 |
|------|------|----------|
| SharedPreferences `crash` | scope / 禁用列表 | 否 |
...
**结论**：用户不点通知则该次崩溃无 UI 留存；logcat 缓冲区有限且重启丢失。**不满足**「记录所有拦截崩溃 + 统计」。

### 拦截时可采集的字段（现状 vs 待建）

| 字段 | 现状 | 待建 |
|------|------|------|
| stack trace | ✓（通知 / Intent） | ✓ 持久化 |
| 异常类 / message | ✓ | ✓ |
| 包名 / 应用名 | ✓ | ✓ |
| 时间戳 | ✗ | ✓ |
| 进程名 / PID / UID | ✗ | ✓ |
...
多后端并行 + root 优先 + 模块 ingest 见 **[crash-log-backends.md](crash-log-backends.md)**（[ADR-008](../decisions/008-multi-backend-crash-log-storage.md)）。本节保留 canonical 与单路径语义摘要。
...
部分 Android 10+ / OEM 上跨包直写 `filesDir` 可能失败（SELinux EACCES；API 30+ 包可见性）。若真机验证失败，增加极简 **`CrashLogProvider`**（仅 `insert`，内部 append 同一 JSONL）。
...
目标 app 进程 (hook)
  └── CrashLogCoordinator.logAsync(event)
        ├─ Phase1: RootSuBackend (tier 0)
        └─ Phase2: ProviderBackend ∥ DirectFsBackend ∥ TargetRelayBackend
...
| **MVP** | `CrashLogger`：hook 回调内异步 append；失败 silent | 全量记录，无 UI |

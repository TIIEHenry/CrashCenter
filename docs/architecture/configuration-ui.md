...
| **Domain Page** | 配置域页面编排，不拥有全局壳层；只处理配置页控件与列表 | `config/ConfigFragment`、`AppToggleAdapter` |
| **Feature State** | 配置页可测试状态、应用列表加载、过滤、排序、prefs 写入 | `ConfigViewModel`、`ConfigUiState`、`AppListRepository` |
...
| 应用列表加载、过滤、排序、prefs 更新、包可见性状态 | `ConfigViewModel` + `AppListRepository` |
...
|------|------|
| `isLoading` | 应用列表加载中 |
| `apps` / `visibleApps` | 原始列表与过滤后列表 |
| `query` | 搜索关键字 |
| `hookFilter` | 全部 / 已应用 / 未应用 |
...
| `emptyReason` | 无应用、无匹配、包可见性受限 |

### `AppListRepository`

`AppListRepository` 封装 `PackageManager`、`SharedPreferences` 和 `PackageVisibilityHelper`：

- `loadInstalledApps(includeSystemUi)`：后台读取并返回 domain model
- `setPackageEnabled(packageName, enabled)`：写入 `package_list`
- `setScopeMode()` / `setHandleSystem()` / `setShowSystemUi()`：写入全局 prefs
- `detectPackageVisibility()`：Android 11+ 包可见性探测

Repository 仍写模块私有 `SharedPreferences`；hook 侧继续通过 XSharedPreferences 读取（见 [scope-and-prefs.md](scope-and-prefs.md)）。
...
|----|------------------------|----------------------------|
| Repository | `AppListRepository` | **`ManagedAppRepository`** |
| 列表数据源 | 全部已安装包 | **`managed_packages`** |
| Hook 写入 | `setPackageEnabled` → `package_list` | **`setInterventionEnabled` + rules JSON** |
...
取代 `AppListRepository` 的 per-app 职责（全局 prefs 方法可保留或上提）：

- `observeManagedApps()` / `addManagedPackages()` / `removeManagedPackage()`
- `setInterventionEnabled()` — 行内 Switch
- `getProfile()` / `saveProfile()` — `intervention_rules` JSON
- `pruneUninstalled()` / `detectPackageVisibility()`

---

## ActivityMain（Legacy 验收基线）

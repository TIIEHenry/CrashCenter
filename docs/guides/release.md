...
主仓库推送 `v*` tag 后，[release.yml](../../.github/workflows/release.yml) 的 **`xposed-module-release`** job 会尝试在模块仓创建 Release（tag `{versionCode}-{versionName}`，附同一 APK）。

**一次性配置**（在 [CrashCenter → Settings → Secrets](https://github.com/TIIEHenry/CrashCenter/settings/secrets/actions)）：
...
3. 回到 CrashCenter → Settings → Secrets → 更新 `XPOSED_REPO_TOKEN`（若重新生成了 token）  
4. 重跑 **LSPosed Module Release (manual)** workflow  

本地自测（勿把 token 贴到聊天）：

```bash
export GH_TOKEN="<你的 classic PAT>"
gh api repos/Xposed-Modules-Repo/nota.android.crash.xp.app --jq .full_name
# 应输出：Xposed-Modules-Repo/nota.android.crash.xp.app
```

未配置时：主仓 Release **仍正常**；模块仓 job 打印 warning 并跳过。

**补发已有版本**（如 v1.0.0 已发主仓、模块仓尚未发布）：

1. 配置好 `XPOSED_REPO_TOKEN`
2. Actions → **LSPosed Module Release (manual)** → Run workflow  
   - `version_name`: `1.0.0`  
   - `version_code`: `1`  
3. 工作流从主仓 Release 下载 APK 并创建模块仓 tag `1-1.0.0`

### 每次发版操作

**自动（推荐）**：配置 `XPOSED_REPO_TOKEN` 后，推送 `vX.Y.Z` 即同时完成双渠道（元数据 `README` 等变更仍需手动 push 模块仓 `main`，仅 Release/APK 自动化）。

**手动（备用）**：未配置 token 或自动 job 失败时，在网页创建 Release：
...
| 现象 | 处理 |
|------|------|
| `Permission denied` / PAT **403** | 组织 SSO 未授权 | Classic PAT + **Configure SSO → Authorize Xposed-Modules-Repo** |
...
| [`.github/workflows/xposed-module-release.yml`](../../.github/workflows/xposed-module-release.yml) | 手动补发模块仓 Release |

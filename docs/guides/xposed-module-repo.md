...
| 触发 | 推送 `v*` tag → CI（含可选模块仓 job） | 同上，或 [xposed-module-release.yml](../../.github/workflows/xposed-module-release.yml) 手动 |
| 仓库内容 | 完整源码 | `README` / `SUMMARY` / `SOURCE_URL` + APK |

## XPOSED_REPO_TOKEN

自动发布模块仓 Release 需在主仓配置 Secret **`XPOSED_REPO_TOKEN`**：

- **Classic** PAT，完整 **`repo`**  scope（非 `public_repo`）
- SSO Authorize 组织 **`Xposed-Modules-Repo`**
- 本地验证：`gh api repos/Xposed-Modules-Repo/nota.android.crash.xp.app --jq '.permissions'` → `"push": true`

详见 [release.md#lsposed-模块仓库自动发布](release.md#lsposed-模块仓库自动发布)。
